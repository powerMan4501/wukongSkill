...
internal static class NativeMethods
{
	...
	internal class PERF_COUNTER_BLOCK
	{
		...
		internal static readonly int SizeOf = Marshal.SizeOf(typeof(PERF_COUNTER_BLOCK));

		public static void ValidateBeforeRead(IntPtr bufferStart, int bufferSize, IntPtr current)
		{
			NativeMethods.ValidateBeforeRead(bufferStart, bufferSize, current, SizeOf);
		}

		public void Validate(int bufferSize)
		{
			if (!System.LocalAppContextSwitches.DoNotValidatePerformanceCounterData && (ByteLength < SizeOf || ByteLength > bufferSize))
			{
	...
	internal class PERF_COUNTER_DEFINITION
	{
		...
		internal static readonly int SizeOf = Marshal.SizeOf(typeof(PERF_COUNTER_DEFINITION));
		...
		public void Validate(int bufferSize)
		{
			if (!System.LocalAppContextSwitches.DoNotValidatePerformanceCounterData && (ByteLength < SizeOf || ByteLength > bufferSize || CounterSize < 0 || CounterOffset < 0 || CounterOffset > bufferSize))
			{
	...
	internal class PERF_DATA_BLOCK
	{
		...
		internal static readonly int SizeOf = Marshal.SizeOf(typeof(PERF_DATA_BLOCK));
		...
		public void Validate(int bufferSize)
		{
			if (!System.LocalAppContextSwitches.DoNotValidatePerformanceCounterData && (TotalByteLength < SizeOf || TotalByteLength > bufferSize || HeaderLength < SizeOf || HeaderLength > TotalByteLength))
			{
	...
	internal class PERF_INSTANCE_DEFINITION
	{
		...
		internal static readonly int SizeOf = Marshal.SizeOf(typeof(PERF_INSTANCE_DEFINITION));
		...
		public void Validate(int bufferSize)
		{
			if (!System.LocalAppContextSwitches.DoNotValidatePerformanceCounterData && (ByteLength < SizeOf || ByteLength > bufferSize || NameOffset < 0 || NameLength < 0 || checked(NameOffset + NameLength) > ByteLength))
			{
	...
	internal class PERF_OBJECT_TYPE
	{
		...
		internal static readonly int SizeOf = Marshal.SizeOf(typeof(PERF_OBJECT_TYPE));
		...
		public void Validate(int bufferSize)
		{
			if (!System.LocalAppContextSwitches.DoNotValidatePerformanceCounterData && (HeaderLength < SizeOf || HeaderLength > TotalByteLength || HeaderLength > DefinitionLength || DefinitionLength < SizeOf || DefinitionLength > TotalByteLength || TotalByteLength > bufferSize || NumCounters < 0 || checked(((NumInstances > 0) ? (NumInstances * PERF_INSTANCE_DEFINITION.SizeOf) : 0) + NumCounters * PERF_COUNTER_DEFINITION.SizeOf) > bufferSize))
			{
	...
	internal class ShellExecuteInfo
	{
		...
		{
			cbSize = Marshal.SizeOf((object)this);
		}
	}
	...
	{
		return SetNamedSecurityInfo(directory, 1u, 20u, IntPtr.Zero, IntPtr.Zero, pDacl, IntPtr.Zero);
	}
	...
	{
		if (!System.LocalAppContextSwitches.DoNotValidatePerformanceCounterData)
		{
	...
	{
		throw new InvalidOperationException(SR.GetString("InvalidPerfData", type.Name));
	}
