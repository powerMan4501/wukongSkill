...
public abstract class HMAC : KeyedHashAlgorithm
{
	...
	public string HashName
	{
		...
		set
		{
			...
			if (m_impl != null && !string.Equals(m_hashName, value, StringComparison.OrdinalIgnoreCase))
			{
				ClearImpl();
			}
			m_hashName = value;
	...
	protected override void HashCore(byte[] rgb, int ib, int cb)
	{
		if (m_impl != null)
		{
			if (!m_hashing)
			{
				m_impl.SetKey(KeyValue);
				m_hashing = true;
			}
			if (cb > 0)
			{
				m_impl.AppendData(rgb, ib, cb);
			}
		}
		else
		{
	...
	protected override byte[] HashFinal()
	{
		if (m_impl != null)
		{
			...
			byte[] array = new byte[HashSizeValue >> 3];
			m_impl.Finish(array);
			m_hashing = false;
			return array;
		}
