...
internal static class AppContextDefaultValues
{
	...
	private static void PopulateDefaultValuesPartial(string platformIdentifier, string profile, int version)
	{
		if (platformIdentifier == ".NETCore" || platformIdentifier == ".NETFramework")
		{
			if (version <= 40601)
			{
				System.LocalAppContext.DefineSwitchDefault("Switch.System.Security.Cryptography.AesCryptoServiceProvider.DontCorrectlyResetDecryptor", initialValue: true);
			}
			if (version <= 40702)
			{
				System.LocalAppContext.DefineSwitchDefault(System.LocalAppContextSwitches.SwitchCryptographyUseLegacyFipsThrow, initialValue: true);
			}
		}
	}
}
