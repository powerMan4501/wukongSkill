...
internal sealed class CompilerScope
{
	...
	private string CurrentLambdaName
	{
		get
		{
			...
			do
			{
				lambdaExpression = Node as LambdaExpression;
			}
	...
	internal void EmitVariableAccess(LambdaCompiler lc, ReadOnlyCollection<ParameterExpression> vars)
	{
		if (NearestHoistedLocals != null)
		{
			...
			if (list.Count > 0)
			{
				...
				lc.EmitConstantArray(list.ToArray());
	...
	private void EmitNewHoistedLocals(LambdaCompiler lc)
	{
		...
		foreach (ParameterExpression variable in _hoistedLocals.Variables)
		{
			...
			{
				int index = lc.Parameters.IndexOf(variable);
	...
	private IList<ParameterExpression> GetVariables()
	{
		...
		foreach (object mergedScope in MergedScopes)
		{
			list.AddRange(GetVariables(mergedScope));
		}
		return list;
	}
	...
	{
		if (scope is LambdaExpression lambdaExpression)
		{
