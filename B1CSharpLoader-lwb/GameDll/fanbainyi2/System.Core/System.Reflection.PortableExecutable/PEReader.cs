...
internal sealed class PEReader : IDisposable
{
	...
	public MetadataReaderProvider ReadEmbeddedPortablePdbDebugDirectoryData(DebugDirectoryEntry entry)
	{
		...
		using AbstractMemoryBlock block = GetDebugDirectoryEntryDataBlock(entry);
		ImmutableArray<byte> image = DecodeEmbeddedPortablePdbDebugDirectoryData(block);
		return MetadataReaderProvider.FromPortablePdbImage(image);
	}
	...
	[SecuritySafeCritical]
	internal unsafe static ImmutableArray<byte> DecodeEmbeddedPortablePdbDebugDirectoryData(AbstractMemoryBlock block)
	{
		BlobReader reader = block.GetReader();
