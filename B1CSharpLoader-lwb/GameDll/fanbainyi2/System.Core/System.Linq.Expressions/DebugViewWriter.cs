...
internal sealed class DebugViewWriter : ExpressionVisitor
{
	...
	private static string FormatBinder(CallSiteBinder binder)
	{
		...
		if (binder is UnaryOperationBinder unaryOperationBinder)
		{
			return "UnaryOperation " + unaryOperationBinder.Operation;
		}
		if (binder is BinaryOperationBinder binaryOperationBinder)
		{
	...
	protected internal override Expression VisitBinary(BinaryExpression node)
	{
		...
		else
		{
			...
			Out(before, text, Flow.Space | Flow.Break);
			if (flag2)
			{
				Out("(", Flow.None);
			}
	...
	{
		Out(string.Format(CultureInfo.CurrentCulture, "{0} {1}<{2}>", new object[3]
		{
			".Lambda",
			GetLambdaName(node),
			node.Type.ToString()
	...
	protected internal override Expression VisitConstant(ConstantExpression node)
	{
		...
		{
			Out(string.Format(CultureInfo.CurrentCulture, "\"{0}\"", new object[1] { value }));
		}
		...
		else
		{
			...
			{
				Out(string.Format(CultureInfo.CurrentCulture, ".Constant<{0}>({1})", new object[2]
				{
					node.Type.ToString(),
					value
				}));
	...
	protected internal override Expression VisitBlock(BlockExpression node)
	{
		...
		{
			Out(string.Format(CultureInfo.CurrentCulture, "<{0}>", new object[1] { node.Type.ToString() }));
		}
	...
	{
		Out(string.Format(CultureInfo.CurrentCulture, ".Extension<{0}>", new object[1] { node.GetType().ToString() }));
		if (node.CanReduce)
		{
	...
	{
		Out(string.Format(CultureInfo.CurrentCulture, ".LabelTarget {0}:", new object[1] { GetLabelTargetName(target) }));
	}
	...
	private string GetLabelTargetName(LabelTarget target)
	{
		...
		{
			return string.Format(CultureInfo.CurrentCulture, "#Label{0}", new object[1] { GetLabelTargetId(target) });
		}
	...
	{
		Out(string.Format(CultureInfo.CurrentCulture, ".Lambda {0}<{1}>", new object[2]
		{
	...
	{
		return string.Format(CultureInfo.CurrentCulture, "'{0}'", new object[1] { name });
	}
