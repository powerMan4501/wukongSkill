...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/AudioExtensions.AudioParameter", "AudioExtensions", UnrealModuleType.Engine)]
public struct FAudioParameter
{
	...
	private static bool FAudioParameter_IsValid;

	private static int FAudioParameter_StructSize;

	public FAudioParameter Copy()
	{
		FAudioParameter result = this;
	...
	public static FAudioParameter FromNative(IntPtr nativeBuffer)
	{
		return new FAudioParameter(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FAudioParameter value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FAudioParameter FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FAudioParameter(nativeBuffer + arrayIndex * FAudioParameter_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FAudioParameter value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FAudioParameter_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FAudioParameter_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/AudioExtensions.AudioParameter");
			return;
		}
	...
	public FAudioParameter(IntPtr nativeStruct)
	{
		if (!FAudioParameter_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/AudioExtensions.AudioParameter");
	...
	static FAudioParameter()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FAudioParameter)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FAudioParameter));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/AudioExtensions.AudioParameter");
		FAudioParameter_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		FAudioParameter_IsValid = intPtr != IntPtr.Zero && ParamName_IsValid && FloatParam_IsValid && BoolParam_IsValid && IntParam_IsValid && ObjectParam_IsValid && StringParam_IsValid && ArrayFloatParam_IsValid && ArrayBoolParam_IsValid && ArrayIntParam_IsValid && ArrayObjectParam_IsValid && ArrayStringParam_IsValid && ParamType_IsValid;
		NativeReflection.LogStructIsValid("/Script/AudioExtensions.AudioParameter", FAudioParameter_IsValid);
	}
}
