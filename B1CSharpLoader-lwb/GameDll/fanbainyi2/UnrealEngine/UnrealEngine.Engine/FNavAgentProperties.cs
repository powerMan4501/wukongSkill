...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/Engine.NavAgentProperties", "Engine", UnrealModuleType.Engine)]
public struct FNavAgentProperties
{
	...
	private static bool FNavAgentProperties_IsValid;

	private static int FNavAgentProperties_StructSize;

	public FNavAgentProperties Copy()
	{
		return this;
	}

	public static FNavAgentProperties FromNative(IntPtr nativeBuffer)
	{
		return new FNavAgentProperties(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FNavAgentProperties value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FNavAgentProperties FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FNavAgentProperties(nativeBuffer + arrayIndex * FNavAgentProperties_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FNavAgentProperties value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FNavAgentProperties_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FNavAgentProperties_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Engine.NavAgentProperties");
			return;
		}
	...
	public FNavAgentProperties(IntPtr nativeStruct)
	{
		if (!FNavAgentProperties_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Engine.NavAgentProperties");
	...
	static FNavAgentProperties()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FNavAgentProperties)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FNavAgentProperties));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/Engine.NavAgentProperties");
		FNavAgentProperties_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		FNavAgentProperties_IsValid = intPtr != IntPtr.Zero && AgentRadius_IsValid && AgentHeight_IsValid && AgentStepHeight_IsValid && NavWalkingSearchHeightScale_IsValid && PreferredNavData_IsValid && CanCrouch_IsValid && CanJump_IsValid && CanWalk_IsValid && CanSwim_IsValid && CanFly_IsValid;
		NativeReflection.LogStructIsValid("/Script/Engine.NavAgentProperties", FNavAgentProperties_IsValid);
	}
}
