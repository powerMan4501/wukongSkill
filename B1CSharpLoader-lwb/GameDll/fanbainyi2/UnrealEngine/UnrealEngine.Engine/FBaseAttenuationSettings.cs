...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/Engine.BaseAttenuationSettings", "Engine", UnrealModuleType.Engine)]
public struct FBaseAttenuationSettings
{
	...
	private static bool FBaseAttenuationSettings_IsValid;

	private static int FBaseAttenuationSettings_StructSize;

	public FBaseAttenuationSettings Copy()
	{
		return this;
	}

	public static FBaseAttenuationSettings FromNative(IntPtr nativeBuffer)
	{
		return new FBaseAttenuationSettings(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FBaseAttenuationSettings value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FBaseAttenuationSettings FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FBaseAttenuationSettings(nativeBuffer + arrayIndex * FBaseAttenuationSettings_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FBaseAttenuationSettings value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FBaseAttenuationSettings_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FBaseAttenuationSettings_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Engine.BaseAttenuationSettings");
			return;
		}
	...
	public FBaseAttenuationSettings(IntPtr nativeStruct)
	{
		if (!FBaseAttenuationSettings_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Engine.BaseAttenuationSettings");
	...
	static FBaseAttenuationSettings()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FBaseAttenuationSettings)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FBaseAttenuationSettings));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/Engine.BaseAttenuationSettings");
		FBaseAttenuationSettings_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		FBaseAttenuationSettings_IsValid = intPtr != IntPtr.Zero && DistanceAlgorithm_IsValid && AttenuationShape_IsValid && dBAttenuationAtMax_IsValid && FalloffMode_IsValid && AttenuationShapeExtents_IsValid && ConeOffset_IsValid && FalloffDistance_IsValid && ConeSphereRadius_IsValid && ConeSphereFalloffDistance_IsValid && CustomAttenuationCurve_IsValid;
		NativeReflection.LogStructIsValid("/Script/Engine.BaseAttenuationSettings", FBaseAttenuationSettings_IsValid);
	}
}
