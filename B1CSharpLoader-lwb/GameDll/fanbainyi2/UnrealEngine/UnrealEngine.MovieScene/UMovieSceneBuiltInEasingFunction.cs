...
[BlueprintType]
[UMetaPath("/Script/MovieScene.MovieSceneBuiltInEasingFunction", "MovieScene", UnrealModuleType.Engine)]
public class UMovieSceneBuiltInEasingFunction : UObject, IMovieSceneEasingFunction, IInterface
{
	...
	protected unsafe float OnEvaluate_Implementation(float Interp)
	{
		CheckDestroyed();
		if (!OnEvaluate_IsValid)
		{
		...
		IntPtr intPtr = new IntPtr(ptr2);
		BlittableTypeMarshaler<float>.ToNative(IntPtr.Add(intPtr, OnEvaluate_Interp_Offset), 0, OnEvaluate_Interp_PropertyAddress.Address, Interp);
		NativeReflection.InvokeFunctionOptimized(base.Address, OnEvaluate_FunctionAddress, intPtr, OnEvaluate_ParamsSize);
		return BlittableTypeMarshaler<float>.FromNative(IntPtr.Add(intPtr, OnEvaluate_ReturnValue_Offset), 0, OnEvaluate_ReturnValue_PropertyAddress.Address);
	}

	static UMovieSceneBuiltInEasingFunction()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(UMovieSceneBuiltInEasingFunction)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(UMovieSceneBuiltInEasingFunction));
	}

	private static void LoadNativeType()
	{
		IntPtr unrealClass = NativeReflection.GetClass("/Script/MovieScene.MovieSceneBuiltInEasingFunction");
