...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/AnimGraphRuntime.AnimNode_ApplyLimits", "AnimGraphRuntime", UnrealModuleType.Engine)]
public struct FAnimNode_ApplyLimits
{
	...
	private static bool AngularOffsets_IsValid;

	private static FFieldAddress AngularOffsets_PropertyAddress;

	private static int AngularOffsets_Offset;

	[UProperty(Flags = (PropFlags)4503599627371077uL)]
	[UMetaPath("/Script/AnimGraphRuntime.AnimNode_ApplyLimits:AngularOffsets")]
	public List<FVector> AngularOffsets;

	private static bool FAnimNode_ApplyLimits_IsValid;

	private static int FAnimNode_ApplyLimits_StructSize;

	public FAnimNode_ApplyLimits Copy()
	{
		FAnimNode_ApplyLimits result = this;
		if (AngularOffsets != null)
		{
			result.AngularOffsets = new List<FVector>(AngularOffsets);
		}
		return result;
	}

	public static FAnimNode_ApplyLimits FromNative(IntPtr nativeBuffer)
	{
		return new FAnimNode_ApplyLimits(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FAnimNode_ApplyLimits value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FAnimNode_ApplyLimits FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FAnimNode_ApplyLimits(nativeBuffer + arrayIndex * FAnimNode_ApplyLimits_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FAnimNode_ApplyLimits value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FAnimNode_ApplyLimits_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FAnimNode_ApplyLimits_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/AnimGraphRuntime.AnimNode_ApplyLimits");
			return;
		}
		new TArrayCopyMarshaler<FVector>(1, AngularOffsets_PropertyAddress, CachedMarshalingDelegates<FVector, BlittableTypeMarshaler<FVector>>.FromNative, CachedMarshalingDelegates<FVector, BlittableTypeMarshaler<FVector>>.ToNative).ToNative(IntPtr.Add(nativeStruct, AngularOffsets_Offset), AngularOffsets);
	...
	public FAnimNode_ApplyLimits(IntPtr nativeStruct)
	{
		if (!FAnimNode_ApplyLimits_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/AnimGraphRuntime.AnimNode_ApplyLimits");
		...
		{
			AngularOffsets = new TArrayCopyMarshaler<FVector>(1, AngularOffsets_PropertyAddress, CachedMarshalingDelegates<FVector, BlittableTypeMarshaler<FVector>>.FromNative, CachedMarshalingDelegates<FVector, BlittableTypeMarshaler<FVector>>.ToNative).FromNative(IntPtr.Add(nativeStruct, AngularOffsets_Offset));
	...
	static FAnimNode_ApplyLimits()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FAnimNode_ApplyLimits)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FAnimNode_ApplyLimits));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/AnimGraphRuntime.AnimNode_ApplyLimits");
		FAnimNode_ApplyLimits_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		NativeReflectionCached.GetPropertyRef(ref AngularOffsets_PropertyAddress, intPtr, "AngularOffsets");
		AngularOffsets_Offset = NativeReflectionCached.GetPropertyOffset(intPtr, "AngularOffsets");
		AngularOffsets_IsValid = NativeReflectionCached.ValidatePropertyClass(intPtr, "AngularOffsets", Classes.FArrayProperty);
		FAnimNode_ApplyLimits_IsValid = intPtr != IntPtr.Zero && AngularOffsets_IsValid && ComponentPose_IsValid && LODThreshold_IsValid && AlphaInputType_IsValid && AlphaBoolEnabled_IsValid && Alpha_IsValid && AlphaScaleBias_IsValid && AlphaBoolBlend_IsValid && AlphaCurveName_IsValid && AlphaScaleBiasClamp_IsValid;
		NativeReflection.LogStructIsValid("/Script/AnimGraphRuntime.AnimNode_ApplyLimits", FAnimNode_ApplyLimits_IsValid);
	}
}
