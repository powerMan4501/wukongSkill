...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/AnimGraphRuntime.AnimNode_SpringBone", "AnimGraphRuntime", UnrealModuleType.Engine)]
public struct FAnimNode_SpringBone
{
	...
	private static bool MaxDisplacement_IsValid;

	private static int MaxDisplacement_Offset;

	[UProperty(Flags = (PropFlags)6755469234274821uL)]
	[UMetaPath("/Script/AnimGraphRuntime.AnimNode_SpringBone:MaxDisplacement")]
	public float MaxDisplacement;

	private static bool FAnimNode_SpringBone_IsValid;

	private static int FAnimNode_SpringBone_StructSize;

	public FAnimNode_SpringBone Copy()
	{
		return this;
	}

	public static FAnimNode_SpringBone FromNative(IntPtr nativeBuffer)
	{
		return new FAnimNode_SpringBone(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FAnimNode_SpringBone value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FAnimNode_SpringBone FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FAnimNode_SpringBone(nativeBuffer + arrayIndex * FAnimNode_SpringBone_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FAnimNode_SpringBone value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FAnimNode_SpringBone_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FAnimNode_SpringBone_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/AnimGraphRuntime.AnimNode_SpringBone");
			return;
		}
		BlittableTypeMarshaler<float>.ToNative(IntPtr.Add(nativeStruct, MaxDisplacement_Offset), MaxDisplacement);
	...
	public FAnimNode_SpringBone(IntPtr nativeStruct)
	{
		if (!FAnimNode_SpringBone_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/AnimGraphRuntime.AnimNode_SpringBone");
		...
		{
			MaxDisplacement = BlittableTypeMarshaler<float>.FromNative(IntPtr.Add(nativeStruct, MaxDisplacement_Offset));
	...
	static FAnimNode_SpringBone()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FAnimNode_SpringBone)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FAnimNode_SpringBone));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/AnimGraphRuntime.AnimNode_SpringBone");
		FAnimNode_SpringBone_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		MaxDisplacement_Offset = NativeReflectionCached.GetPropertyOffset(intPtr, "MaxDisplacement");
		MaxDisplacement_IsValid = NativeReflectionCached.ValidatePropertyClass(intPtr, "MaxDisplacement", Classes.FFloatProperty);
		FAnimNode_SpringBone_IsValid = intPtr != IntPtr.Zero && MaxDisplacement_IsValid && ComponentPose_IsValid && LODThreshold_IsValid && AlphaInputType_IsValid && AlphaBoolEnabled_IsValid && Alpha_IsValid && AlphaScaleBias_IsValid && AlphaBoolBlend_IsValid && AlphaCurveName_IsValid && AlphaScaleBiasClamp_IsValid;
		NativeReflection.LogStructIsValid("/Script/AnimGraphRuntime.AnimNode_SpringBone", FAnimNode_SpringBone_IsValid);
	}
}
