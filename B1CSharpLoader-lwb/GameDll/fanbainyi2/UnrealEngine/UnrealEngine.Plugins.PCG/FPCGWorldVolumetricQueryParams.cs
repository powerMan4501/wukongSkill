...
[UStruct(Flags = 4097)]
[BlueprintType]
[UMetaPath("/Script/PCG.PCGWorldVolumetricQueryParams", "PCG", UnrealModuleType.EnginePlugin)]
public struct FPCGWorldVolumetricQueryParams
{
	...
	private static bool SearchForOverlap_IsValid;

	private static FFieldAddress SearchForOverlap_PropertyAddress;

	private static int SearchForOverlap_Offset;

	[UProperty(Flags = (PropFlags)6755469234274821uL)]
	[UMetaPath("/Script/PCG.PCGWorldVolumetricQueryParams:bSearchForOverlap")]
	public bool SearchForOverlap;

	private static bool FPCGWorldVolumetricQueryParams_IsValid;

	private static int FPCGWorldVolumetricQueryParams_StructSize;

	public FPCGWorldVolumetricQueryParams Copy()
	{
		return this;
	}

	public static FPCGWorldVolumetricQueryParams FromNative(IntPtr nativeBuffer)
	{
		return new FPCGWorldVolumetricQueryParams(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FPCGWorldVolumetricQueryParams value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FPCGWorldVolumetricQueryParams FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FPCGWorldVolumetricQueryParams(nativeBuffer + arrayIndex * FPCGWorldVolumetricQueryParams_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FPCGWorldVolumetricQueryParams value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FPCGWorldVolumetricQueryParams_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FPCGWorldVolumetricQueryParams_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/PCG.PCGWorldVolumetricQueryParams");
			return;
		}
		BoolMarshaler.ToNative(IntPtr.Add(nativeStruct, SearchForOverlap_Offset), 0, SearchForOverlap_PropertyAddress.Address, SearchForOverlap);
	...
	public FPCGWorldVolumetricQueryParams(IntPtr nativeStruct)
	{
		if (!FPCGWorldVolumetricQueryParams_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/PCG.PCGWorldVolumetricQueryParams");
			SearchForOverlap = false;
			IgnorePCGHits = false;
		...
		{
			SearchForOverlap = BoolMarshaler.FromNative(IntPtr.Add(nativeStruct, SearchForOverlap_Offset), 0, SearchForOverlap_PropertyAddress.Address);
	...
	static FPCGWorldVolumetricQueryParams()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FPCGWorldVolumetricQueryParams)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FPCGWorldVolumetricQueryParams));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/PCG.PCGWorldVolumetricQueryParams");
		FPCGWorldVolumetricQueryParams_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		NativeReflectionCached.GetPropertyRef(ref SearchForOverlap_PropertyAddress, intPtr, "bSearchForOverlap");
		SearchForOverlap_Offset = NativeReflectionCached.GetPropertyOffset(intPtr, "bSearchForOverlap");
		SearchForOverlap_IsValid = NativeReflectionCached.ValidatePropertyClass(intPtr, "bSearchForOverlap", Classes.FBoolProperty);
		FPCGWorldVolumetricQueryParams_IsValid = intPtr != IntPtr.Zero && SearchForOverlap_IsValid && IgnorePCGHits_IsValid && IgnoreSelfHits_IsValid && CollisionChannel_IsValid && TraceComplex_IsValid && ActorTagFilter_IsValid && ActorTagsList_IsValid && IgnoreLandscapeHits_IsValid && GetReferenceToActorHit_IsValid;
		NativeReflection.LogStructIsValid("/Script/PCG.PCGWorldVolumetricQueryParams", FPCGWorldVolumetricQueryParams_IsValid);
	}
}
