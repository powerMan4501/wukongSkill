...
[BlueprintType]
[UMetaPath("/Script/Synthesis.SourceEffectPhaserPreset", "Synthesis", UnrealModuleType.EnginePlugin)]
public class USourceEffectPhaserPreset : USoundEffectSourcePreset
{
	...
	[UMetaPath("/Script/Synthesis.SourceEffectPhaserPreset:Settings")]
	public FSourceEffectPhaserSettings Settings
	{
		get
		{
			...
			if (!Settings_IsValid)
			{
				NativeReflection.LogInvalidPropertyAccessed("/Script/Synthesis.SourceEffectPhaserPreset:Settings");
				return default(FSourceEffectPhaserSettings);
			}
			return FSourceEffectPhaserSettings.FromNative(IntPtr.Add(base.Address, Settings_Offset));
		}
		set
		{
			CheckDestroyed();
			...
			{
				FSourceEffectPhaserSettings.ToNative(IntPtr.Add(base.Address, Settings_Offset), value);
			}
		}
	}

	[UFunction(Flags = 71435265u)]
	[UMetaPath("/Script/Synthesis.SourceEffectPhaserPreset:SetSettings")]
	public unsafe void SetSettings(FSourceEffectPhaserSettings InSettings)
	{
		CheckDestroyed();
		if (!SetSettings_IsValid)
		{
			NativeReflection.LogInvalidFunctionAccessed("/Script/Synthesis.SourceEffectPhaserPreset:SetSettings");
			return;
		}
		...
		FSourceEffectPhaserSettings.ToNative(IntPtr.Add(intPtr, SetSettings_InSettings_Offset), 0, SetSettings_InSettings_PropertyAddress.Address, InSettings);
		NativeReflection.InvokeFunctionOptimized(base.Address, SetSettings_FunctionAddress, intPtr, SetSettings_ParamsSize);
	}

	static USourceEffectPhaserPreset()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(USourceEffectPhaserPreset)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(USourceEffectPhaserPreset));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetClass("/Script/Synthesis.SourceEffectPhaserPreset");
		...
		NativeReflection.LogFunctionIsValid("/Script/Synthesis.SourceEffectPhaserPreset:SetSettings", SetSettings_IsValid);
	}
}
