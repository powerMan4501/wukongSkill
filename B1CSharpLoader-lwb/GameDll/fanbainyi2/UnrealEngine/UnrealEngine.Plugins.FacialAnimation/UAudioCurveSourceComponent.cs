...
namespace UnrealEngine.Plugins.FacialAnimation;

[UClass(Flags = (ClassFlags)821035172uL, Config = "Engine")]
[BlueprintType]
[UMetaPath("/Script/FacialAnimation.AudioCurveSourceComponent", "FacialAnimation", UnrealModuleType.EnginePlugin)]
public class UAudioCurveSourceComponent : UAudioComponent, ICurveSourceInterface, IInterface
{
	private static bool CurveSourceBindingName_IsValid;

	private static int CurveSourceBindingName_Offset;

	private static bool CurveSyncOffset_IsValid;

	private static int CurveSyncOffset_Offset;
	...
	[UMetaPath("/Script/FacialAnimation.AudioCurveSourceComponent:CurveSourceBindingName")]
	public FName CurveSourceBindingName
	{
		get
		{
			CheckDestroyed();
			if (!CurveSourceBindingName_IsValid)
			{
				NativeReflection.LogInvalidPropertyAccessed("/Script/FacialAnimation.AudioCurveSourceComponent:CurveSourceBindingName");
				return default(FName);
			}
			return BlittableTypeMarshaler<FName>.FromNative(IntPtr.Add(base.Address, CurveSourceBindingName_Offset));
		}
		set
		{
			CheckDestroyed();
			...
			{
				BlittableTypeMarshaler<FName>.ToNative(IntPtr.Add(base.Address, CurveSourceBindingName_Offset), value);
			}
		}
	}

	[UProperty(Flags = (PropFlags)6755469234274821uL)]
	[UMetaPath("/Script/FacialAnimation.AudioCurveSourceComponent:CurveSyncOffset")]
	public float CurveSyncOffset
	{
		get
		{
			CheckDestroyed();
			if (!CurveSyncOffset_IsValid)
			{
				NativeReflection.LogInvalidPropertyAccessed("/Script/FacialAnimation.AudioCurveSourceComponent:CurveSyncOffset");
				return 0f;
			}
			return BlittableTypeMarshaler<float>.FromNative(IntPtr.Add(base.Address, CurveSyncOffset_Offset));
		}
		set
		{
			CheckDestroyed();
			...
			{
				BlittableTypeMarshaler<float>.ToNative(IntPtr.Add(base.Address, CurveSyncOffset_Offset), value);
			}
		}
	}
	...
	protected unsafe float GetCurveValue_Implementation(FName CurveName)
	{
		CheckDestroyed();
		if (!GetCurveValue_IsValid)
		{
	...
	protected unsafe void GetCurves_Implementation(out List<FNamedCurveValue> OutValues)
	{
		CheckDestroyed();
		if (!GetCurves_IsValid)
		{
	...
	protected unsafe FName GetBindingName_Implementation()
	{
		CheckDestroyed();
		if (!GetBindingName_IsValid)
		{
		...
		Unsafe.InitBlockUnaligned(ptr2, 0, (uint)GetBindingName_ParamsSize);
		IntPtr intPtr = new IntPtr(ptr2);
		NativeReflection.InvokeFunctionOptimized(base.Address, GetBindingName_FunctionAddress, intPtr, GetBindingName_ParamsSize);
		return BlittableTypeMarshaler<FName>.FromNative(IntPtr.Add(intPtr, GetBindingName_ReturnValue_Offset), 0, GetBindingName_ReturnValue_PropertyAddress.Address);
	}

	static UAudioCurveSourceComponent()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(UAudioCurveSourceComponent)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(UAudioCurveSourceComponent));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetClass("/Script/FacialAnimation.AudioCurveSourceComponent");
		CurveSourceBindingName_Offset = NativeReflectionCached.GetPropertyOffset(intPtr, "CurveSourceBindingName");
		CurveSourceBindingName_IsValid = NativeReflectionCached.ValidatePropertyClass(intPtr, "CurveSourceBindingName", Classes.FNameProperty);
		CurveSyncOffset_Offset = NativeReflectionCached.GetPropertyOffset(intPtr, "CurveSyncOffset");
		CurveSyncOffset_IsValid = NativeReflectionCached.ValidatePropertyClass(intPtr, "CurveSyncOffset", Classes.FFloatProperty);
		...
		GetCurves_FunctionAddress = NativeReflectionCached.GetFunction(intPtr, "GetCurves");
		...
		GetBindingName_FunctionAddress = NativeReflectionCached.GetFunction(intPtr, "GetBindingName");
