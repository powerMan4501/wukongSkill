...
[BlueprintType]
[Blueprintable]
[UMetaPath("/Script/DataprepCore.DataprepIntegerFetcher", "DataprepCore", UnrealModuleType.EnginePlugin)]
public class UDataprepIntegerFetcher : UDataprepFetcher
{
	...
	[UMetaPath("/Script/DataprepCore.DataprepIntegerFetcher:Fetch")]
	public unsafe int Fetch(UObject Object, out bool bOutFetchSucceded)
	{
		CheckDestroyed();
		if (!Fetch_IsValid)
		{
			NativeReflection.LogInvalidFunctionAccessed("/Script/DataprepCore.DataprepIntegerFetcher:Fetch");
		...
		return BlittableTypeMarshaler<int>.FromNative(IntPtr.Add(intPtr, Fetch_ReturnValue_Offset), 0, Fetch_ReturnValue_PropertyAddress.Address);
	}

	protected unsafe virtual int Fetch_Implementation(UObject Object, out bool bOutFetchSucceded)
	{
		CheckDestroyed();
		if (!Fetch_IsValid)
		{
	...
	static UDataprepIntegerFetcher()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(UDataprepIntegerFetcher)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(UDataprepIntegerFetcher));
	}

	private static void LoadNativeType()
	{
		IntPtr unrealClass = NativeReflection.GetClass("/Script/DataprepCore.DataprepIntegerFetcher");
		...
		Fetch_ReturnValue_IsValid = NativeReflectionCached.ValidatePropertyClass(Fetch_FunctionAddress, "ReturnValue", Classes.FIntProperty);
		...
		NativeReflection.LogFunctionIsValid("/Script/DataprepCore.DataprepIntegerFetcher:Fetch", Fetch_IsValid);
	}
}
