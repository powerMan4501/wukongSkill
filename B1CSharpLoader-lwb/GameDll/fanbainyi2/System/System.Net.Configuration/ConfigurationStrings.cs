...
internal static class ConfigurationStrings
{
	...
	{
		return string.Format(CultureInfo.InvariantCulture, "{0}/{1}", new object[2] { "system.net", sectionName });
	}

	private static string GetSectionPath(string sectionName, string subSectionName)
	{
		return string.Format(CultureInfo.InvariantCulture, "{0}/{1}/{2}", new object[3] { "system.net", sectionName, subSectionName });
	}
}
