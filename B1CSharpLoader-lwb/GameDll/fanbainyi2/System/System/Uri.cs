...
public class Uri : ISerializable
{
		...
		if (s_ManagerRef == null)
		{
			lock (s_IntranetLock)
			{
				if (s_ManagerRef == null)
				{
					...
					UnsafeNclNativeMethods.CoCreateInstance(ref clsid, IntPtr.Zero, 21, ref iid, out var o);
					s_ManagerRef = o as IInternetSecurityManager;
				}
			}
		}
		try
	...
	private static UriFormatException GetException(ParsingError err)
	{
		return err switch
		{
			...
			ParsingError.BadFormat => new UriFormatException(SR.GetString("net_uri_BadFormat")), 
			ParsingError.BadScheme => new UriFormatException(SR.GetString("net_uri_BadScheme")), 
			ParsingError.BadAuthority => new UriFormatException(SR.GetString("net_uri_BadAuthority")), 
			ParsingError.EmptyUriString => new UriFormatException(SR.GetString("net_uri_EmptyUri")), 
			ParsingError.SchemeLimit => new UriFormatException(SR.GetString("net_uri_SchemeLimit")), 
			ParsingError.SizeLimit => new UriFormatException(SR.GetString("net_uri_SizeLimit")), 
			ParsingError.MustRootedPath => new UriFormatException(SR.GetString("net_uri_MustRootedPath")), 
			ParsingError.BadHostName => new UriFormatException(SR.GetString("net_uri_BadHostName")), 
			ParsingError.NonEmptyHost => new UriFormatException(SR.GetString("net_uri_BadFormat")), 
			ParsingError.BadPort => new UriFormatException(SR.GetString("net_uri_BadPort")), 
			ParsingError.BadAuthorityTerminator => new UriFormatException(SR.GetString("net_uri_BadAuthorityTerminator")), 
			ParsingError.CannotCreateRelative => new UriFormatException(SR.GetString("net_uri_CannotCreateRelative")), 
			_ => new UriFormatException(SR.GetString("net_uri_BadFormat")), 
		};
	}
	...
	private static void InitializeUriConfig()
	{
		...
		lock (InitializeLock)
		{
			...
			if (section != null)
			{
				...
				SetEscapedDotSlashSettings(section, UriSchemeWs);
				SetEscapedDotSlashSettings(section, UriSchemeWss);
			}
	...
	private unsafe ParsingError PrivateParseMinimal()
	{
		...
		fixed (char* ptr = ((m_iriParsing && (m_Flags & Flags.HasUnicode) != Flags.Zero && (m_Flags & Flags.HostUnicodeNormalized) == Flags.Zero) ? m_originalUnicodeString : m_String))
		{
			...
			if ((m_Flags & (Flags.DosPath | Flags.UncPath)) == Flags.Zero)
			{
				if (num + 2 <= num2)
				{
					char c2 = ptr[(int)num];
					...
					{
						if (m_iriParsing && (m_Flags & Flags.HasUnicode) != Flags.Zero && (m_Flags & Flags.HostUnicodeNormalized) == Flags.Zero)
						{
							m_String = m_String.Substring(0, num);
						}
	...
	private unsafe char[] GetCanonicalPath(char[] dest, ref int pos, UriFormat formatAs)
	{
		...
		if (InFact(Flags.ShouldBeCompressed))
		{
			...
			if (formatAs == UriFormat.UriEscaped && NotAny(Flags.UserEscaped) && InFact(Flags.E_PathNotCanonical))
			{
				...
				dest = UriHelper.EscapeString(input, 0, end - pos, dest, ref pos, isUriString: true, '?', '#', IsImplicitFile ? '\uffff' : '%');
				end = pos;
			}
		}
	...
	private void InitializeUri(ParsingError err, UriKind uriKind, out UriFormatException e)
	{
		...
		if (m_Syntax != null)
		{
			if (m_Syntax.IsSimple)
			{
				...
				if (!(m_iriParsing && flag) || (uriKind != UriKind.Absolute && err != ParsingError.None))
				{
					return;
				}
				try
