...
internal class HeaderInfoTable
{
	...
	private static HeaderParser SetCookieParser;
	...
	{
		return ParseValueHelper(value, isSetCookie: false);
	}

	private static string[] ParseSetCookieValue(string value)
	{
		return ParseValueHelper(value, isSetCookie: true);
	}
	...
	{
		string[] array = singleValue.Split(';');
	...
	static HeaderInfoTable()
	{
		...
		SetCookieParser = ParseSetCookieValue;
		HeaderInfo[] array = new HeaderInfo[52]
		{
			...
			new HeaderInfo("Set-Cookie", requestRestricted: false, responseRestricted: false, multi: true, SetCookieParser),
			new HeaderInfo("Set-Cookie2", requestRestricted: false, responseRestricted: false, multi: true, SetCookieParser),
