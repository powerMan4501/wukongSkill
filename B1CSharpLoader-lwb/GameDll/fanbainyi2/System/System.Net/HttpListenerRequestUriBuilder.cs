...
internal sealed class HttpListenerRequestUriBuilder
{
	...
	private void BuildRequestUriUsingCookedPath()
	{
		...
		{
			LogWarning("BuildRequestUriUsingCookedPath", "net_log_listener_cant_create_uri", cookedUriScheme, cookedUriHost, cookedUriPath, cookedUriQuery);
		}
	}
	...
	private void BuildRequestUriUsingRawPath()
	{
		...
		if (!flag)
		{
			LogWarning("BuildRequestUriUsingRawPath", "net_log_listener_cant_create_uri", cookedUriScheme, cookedUriHost, rawPath, cookedUriQuery);
		}
	}
	...
	private ParsingResult BuildRequestUriUsingRawPath(Encoding encoding)
	{
		...
		if (parsingResult != ParsingResult.Success)
		{
			LogWarning("BuildRequestUriUsingRawPath", "net_log_listener_cant_convert_raw_path", rawPath, encoding.EncodingName);
		}
	...
	private bool AppendUnicodeCodePointValuePercentEncoded(string codePoint)
	{
		...
		{
			LogWarning("AppendUnicodeCodePointValuePercentEncoded", "net_log_listener_cant_convert_percent_value", codePoint);
			return false;
		}
		string text = null;
		try
		...
		{
			LogWarning("AppendUnicodeCodePointValuePercentEncoded", "net_log_listener_cant_convert_to_utf8", text, ex2.Message);
		}
		return false;
	}
	...
	private bool AddPercentEncodedOctetToRawOctetsList(Encoding encoding, string escapedCharacter)
	{
		...
		{
			LogWarning("AddPercentEncodedOctetToRawOctetsList", "net_log_listener_cant_convert_percent_value", escapedCharacter);
			return false;
		}
	...
	private bool EmptyDecodeAndAppendRawOctetsList(Encoding encoding)
	{
		...
		{
			LogWarning("EmptyDecodeAndAppendRawOctetsList", "net_log_listener_cant_convert_bytes", GetOctetsAsString(rawOctets), ex.Message);
		}
		catch (EncoderFallbackException ex2)
		{
			LogWarning("EmptyDecodeAndAppendRawOctetsList", "net_log_listener_cant_convert_to_utf8", text, ex2.Message);
		}
		return false;
	}
