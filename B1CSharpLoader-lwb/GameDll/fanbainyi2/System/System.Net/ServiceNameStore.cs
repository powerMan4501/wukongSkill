...
internal class ServiceNameStore
{
	...
	public bool Add(string uriPrefix)
	{
		...
		foreach (string text in array2)
		{
			if (AddSingleServiceName(text))
			{
				...
				if (Logging.On)
				{
					Logging.PrintInfo(Logging.HttpListener, "ServiceNameStore#" + ValidationHelper.HashString(this) + "::Add() " + SR.GetString("net_log_listener_spn_add", text, uriPrefix));
				}
			}
		}
		if (flag)
		...
		{
			Logging.PrintInfo(Logging.HttpListener, "ServiceNameStore#" + ValidationHelper.HashString(this) + "::Add() " + SR.GetString("net_log_listener_spn_not_add", uriPrefix));
		}
		return flag;
	}
	...
	public bool Remove(string uriPrefix)
	{
		...
		if (Logging.On)
		{
			if (flag)
			{
				Logging.PrintInfo(Logging.HttpListener, "ServiceNameStore#" + ValidationHelper.HashString(this) + "::Remove() " + SR.GetString("net_log_listener_spn_remove", inputServiceName, uriPrefix));
			}
			else
			{
				Logging.PrintInfo(Logging.HttpListener, "ServiceNameStore#" + ValidationHelper.HashString(this) + "::Remove() " + SR.GetString("net_log_listener_spn_not_remove", uriPrefix));
			}
		}
		return flag;
	}
