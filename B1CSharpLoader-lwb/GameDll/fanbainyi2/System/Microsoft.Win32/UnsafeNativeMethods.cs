...
internal static class UnsafeNativeMethods
{
	...
	private static extern int _AppPolicyGetClrCompat(IntPtr processToken, out AppPolicyClrCompat appPolicyClrCompat);
	...
	private static bool _IsPackagedProcess()
	{
		...
		if (flag && DoesWin32MethodExist("kernel32.dll", "AppPolicyGetClrCompat"))
		{
			if (_AppPolicyGetClrCompat(GetCurrentProcessToken(), out var appPolicyClrCompat) == 0)
			{
				return appPolicyClrCompat == AppPolicyClrCompat.AppPolicyClrCompat_Universal;
			}
			return false;
		}
		if (flag && DoesWin32MethodExist("kernel32.dll", "GetCurrentPackageId"))
		{
			int pBufferLength = 0;
