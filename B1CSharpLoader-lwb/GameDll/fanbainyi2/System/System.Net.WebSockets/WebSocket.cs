...
public abstract class WebSocket : IDisposable
{
	...
	public static TimeSpan DefaultKeepAliveInterval
	{
		get
		{
			if (!defaultKeepAliveInterval.HasValue)
			{
	...
	public static ArraySegment<byte> CreateClientBuffer(int receiveBufferSize, int sendBufferSize)
	{
		...
		return WebSocketBuffer.CreateInternalBufferArraySegment(receiveBufferSize, sendBufferSize, isServerBuffer: false);
	}
	...
	public static ArraySegment<byte> CreateServerBuffer(int receiveBufferSize)
	{
		...
		return WebSocketBuffer.CreateInternalBufferArraySegment(receiveBufferSize, 16, isServerBuffer: true);
	}
	...
	public static WebSocket CreateClientWebSocket(Stream innerStream, string subProtocol, int receiveBufferSize, int sendBufferSize, TimeSpan keepAliveInterval, bool useZeroMaskingKey, ArraySegment<byte> internalBuffer)
	{
		...
		WebSocketBuffer.Validate(internalBuffer.Count, receiveBufferSize, sendBufferSize, isServerBuffer: false);
	...
	{
		WebRequest.RegisterPrefix(Uri.UriSchemeWs + ":", new WebSocketHttpRequestCreator(usingHttps: false));
		WebRequest.RegisterPrefix(Uri.UriSchemeWss + ":", new WebSocketHttpRequestCreator(usingHttps: true));
	}
	...
	protected static void ThrowOnInvalidState(WebSocketState state, params WebSocketState[] validStates)
	{
		...
		throw new WebSocketException(SR.GetString("net_WebSockets_InvalidState", state, text));
	}
