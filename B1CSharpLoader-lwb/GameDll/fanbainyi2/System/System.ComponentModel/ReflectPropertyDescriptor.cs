	...
	private EventDescriptor ChangedEventValue
	{
		get
		{
			if (!state[BitChangedQueried])
			{
				...
				realChangedEvent = TypeDescriptor.GetEvents(ComponentType)[string.Format(CultureInfo.InvariantCulture, "{0}Changed", new object[1] { Name })];
			}
	...
	private MethodInfo ResetMethodValue
	{
		get
		{
			if (!state[BitResetQueried])
			{
				...
				Type[] args = ((!(receiverType == null)) ? new Type[1] { receiverType } : argsNone);
	...
	public override object GetValue(object component)
	{
		...
		if (component != null)
		{
			...
			catch (Exception innerException)
			{
				...
				throw new TargetInvocationException(SR.GetString("ErrorPropertyAccessorException", Name, text, message), innerException);
			}
		}
		return null;
	}
