...
[BlueprintType]
[Blueprintable]
[UMetaPath("/Script/SimpleCharts.LineChart", "SimpleCharts", UnrealModuleType.GamePlugin)]
public class ULineChart : UChartBase
{
	...
	private TArrayReadWriteMarshaler<FLineSeries> Series_MarshalerCached;
	...
	[UMetaPath("/Script/SimpleCharts.LineChart:Series")]
	public TArrayReadWrite<FLineSeries> Series
	{
		get
		{
			CheckDestroyed();
			if (!Series_IsValid)
			{
				NativeReflection.LogInvalidPropertyAccessed("/Script/SimpleCharts.LineChart:Series");
				return null;
			}
			if (Series_MarshalerCached == null)
			{
				Series_MarshalerCached = new TArrayReadWriteMarshaler<FLineSeries>(1, Series_PropertyAddress, CachedMarshalingDelegates<FLineSeries, FLineSeries>.FromNative, CachedMarshalingDelegates<FLineSeries, FLineSeries>.ToNative);
			}
	...
	[UMetaPath("/Script/SimpleCharts.LineChart:SetSeries")]
	public unsafe void SetSeries(List<FLineSeries> SeriesData)
	{
		CheckDestroyed();
		if (!SetSeries_IsValid)
		{
			NativeReflection.LogInvalidFunctionAccessed("/Script/SimpleCharts.LineChart:SetSeries");
			return;
		}
		...
		new TArrayCopyMarshaler<FLineSeries>(1, SetSeries_SeriesData_PropertyAddress, CachedMarshalingDelegates<FLineSeries, FLineSeries>.FromNative, CachedMarshalingDelegates<FLineSeries, FLineSeries>.ToNative).ToNative(IntPtr.Add(intPtr, SetSeries_SeriesData_Offset), SeriesData);
	...
	[UMetaPath("/Script/SimpleCharts.LineChart:SetLegend")]
	public unsafe void SetLegend(bool show, EPosition position, EOrient orient, FLinearColor color, int fontSize)
	{
		...
		{
			NativeReflection.LogInvalidFunctionAccessed("/Script/SimpleCharts.LineChart:SetLegend");
			return;
		}
	...
	static ULineChart()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(ULineChart)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(ULineChart));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetClass("/Script/SimpleCharts.LineChart");
		...
		NativeReflection.LogFunctionIsValid("/Script/SimpleCharts.LineChart:SetSeries", SetSeries_IsValid);
		...
		NativeReflection.LogFunctionIsValid("/Script/SimpleCharts.LineChart:SetLegend", SetLegend_IsValid);
	}
}
