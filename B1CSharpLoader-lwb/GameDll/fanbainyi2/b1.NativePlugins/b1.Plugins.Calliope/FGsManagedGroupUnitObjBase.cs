...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/Calliope.GsManagedGroupUnitObjBase", "Calliope", UnrealModuleType.GamePlugin)]
public struct FGsManagedGroupUnitObjBase
{
	...
	private static bool FGsManagedGroupUnitObjBase_IsValid;

	private static int FGsManagedGroupUnitObjBase_StructSize;

	public FGsManagedGroupUnitObjBase Copy()
	{
		return this;
	}

	public static FGsManagedGroupUnitObjBase FromNative(IntPtr nativeBuffer)
	{
		return new FGsManagedGroupUnitObjBase(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FGsManagedGroupUnitObjBase value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FGsManagedGroupUnitObjBase FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FGsManagedGroupUnitObjBase(nativeBuffer + arrayIndex * FGsManagedGroupUnitObjBase_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FGsManagedGroupUnitObjBase value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FGsManagedGroupUnitObjBase_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FGsManagedGroupUnitObjBase_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Calliope.GsManagedGroupUnitObjBase");
			return;
		}
	...
	public FGsManagedGroupUnitObjBase(IntPtr nativeStruct)
	{
		if (!FGsManagedGroupUnitObjBase_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Calliope.GsManagedGroupUnitObjBase");
	...
	static FGsManagedGroupUnitObjBase()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FGsManagedGroupUnitObjBase)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FGsManagedGroupUnitObjBase));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/Calliope.GsManagedGroupUnitObjBase");
		FGsManagedGroupUnitObjBase_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		FGsManagedGroupUnitObjBase_IsValid = intPtr != IntPtr.Zero && GroupTag_IsValid && NeedStateManaged_IsValid && ParamType_IsValid && ConfigGuid_IsValid && AliasName_IsValid;
		NativeReflection.LogStructIsValid("/Script/Calliope.GsManagedGroupUnitObjBase", FGsManagedGroupUnitObjBase_IsValid);
	}
}
