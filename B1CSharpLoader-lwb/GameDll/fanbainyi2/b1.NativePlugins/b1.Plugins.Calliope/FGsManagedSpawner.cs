...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/Calliope.GsManagedSpawner", "Calliope", UnrealModuleType.GamePlugin)]
public struct FGsManagedSpawner
{
	...
	[UMetaPath("/Script/Calliope.GsManagedSpawner:Actor")]
	...
	[UProperty(Flags = (PropFlags)6755469234274821uL)]
	[UMetaPath("/Script/Calliope.GsManagedSpawner:State")]
	public EGsSpawnerState State;

	private static bool FGsManagedSpawner_IsValid;

	private static int FGsManagedSpawner_StructSize;

	public FGsManagedSpawner Copy()
	{
		return this;
	}

	public static FGsManagedSpawner FromNative(IntPtr nativeBuffer)
	{
		return new FGsManagedSpawner(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FGsManagedSpawner value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FGsManagedSpawner FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FGsManagedSpawner(nativeBuffer + arrayIndex * FGsManagedSpawner_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FGsManagedSpawner value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FGsManagedSpawner_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FGsManagedSpawner_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Calliope.GsManagedSpawner");
			return;
		}
	...
	public FGsManagedSpawner(IntPtr nativeStruct)
	{
		if (!FGsManagedSpawner_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Calliope.GsManagedSpawner");
	...
	static FGsManagedSpawner()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FGsManagedSpawner)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FGsManagedSpawner));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/Calliope.GsManagedSpawner");
		FGsManagedSpawner_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		FGsManagedSpawner_IsValid = intPtr != IntPtr.Zero && Actor_IsValid && State_IsValid && ActorGuid_IsValid && NeedStateManaged_IsValid && ParamType_IsValid && ConfigGuid_IsValid && AliasName_IsValid;
		NativeReflection.LogStructIsValid("/Script/Calliope.GsManagedSpawner", FGsManagedSpawner_IsValid);
	}
}
