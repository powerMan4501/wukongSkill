...
[UStruct(Flags = 4609)]
[BlueprintType]
[UMetaPath("/Script/Calliope.GsManagedOverlap", "Calliope", UnrealModuleType.GamePlugin)]
public struct FGsManagedOverlap
{
	...
	[UMetaPath("/Script/Calliope.GsManagedOverlap:Actor")]
	...
	[UProperty(Flags = (PropFlags)6755469234274821uL)]
	[UMetaPath("/Script/Calliope.GsManagedOverlap:State")]
	public EGsOverlapState State;

	private static bool FGsManagedOverlap_IsValid;

	private static int FGsManagedOverlap_StructSize;

	public FGsManagedOverlap Copy()
	{
		return this;
	}

	public static FGsManagedOverlap FromNative(IntPtr nativeBuffer)
	{
		return new FGsManagedOverlap(nativeBuffer);
	}

	public static void ToNative(IntPtr nativeBuffer, FGsManagedOverlap value)
	{
		value.ToNative(nativeBuffer);
	}

	public static FGsManagedOverlap FromNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop)
	{
		return new FGsManagedOverlap(nativeBuffer + arrayIndex * FGsManagedOverlap_StructSize);
	}

	public static void ToNative(IntPtr nativeBuffer, int arrayIndex, IntPtr prop, FGsManagedOverlap value)
	{
		value.ToNative(nativeBuffer + arrayIndex * FGsManagedOverlap_StructSize);
	}

	public void ToNative(IntPtr nativeStruct)
	{
		if (!FGsManagedOverlap_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Calliope.GsManagedOverlap");
			return;
		}
	...
	public FGsManagedOverlap(IntPtr nativeStruct)
	{
		if (!FGsManagedOverlap_IsValid)
		{
			NativeReflection.LogInvalidStructAccessed("/Script/Calliope.GsManagedOverlap");
	...
	static FGsManagedOverlap()
	{
		if (UnrealTypes.CanLazyLoadNativeType(typeof(FGsManagedOverlap)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FGsManagedOverlap));
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetStruct("/Script/Calliope.GsManagedOverlap");
		FGsManagedOverlap_StructSize = NativeReflection.GetStructSize(intPtr);
		...
		FGsManagedOverlap_IsValid = intPtr != IntPtr.Zero && Actor_IsValid && State_IsValid && ActorGuid_IsValid && NeedStateManaged_IsValid && ParamType_IsValid && ConfigGuid_IsValid && AliasName_IsValid;
		NativeReflection.LogStructIsValid("/Script/Calliope.GsManagedOverlap", FGsManagedOverlap_IsValid);
	}
}
