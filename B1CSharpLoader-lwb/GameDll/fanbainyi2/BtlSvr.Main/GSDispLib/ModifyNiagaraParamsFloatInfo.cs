...
public class ModifyNiagaraParamsFloatInfo
{
	private BUC_DispLibUtil_ModifyNiagaraParams_Float SourceStruct;
	...
	public ModifyNiagaraParamsFloatInfo(BUC_DispLibUtil_ModifyNiagaraParams_Float _SourceStruct, FVector Velocity, float StartAge)
	{
	...
	protected virtual void Init(BUC_DispLibUtil_ModifyNiagaraParams_Float _SourceStruct, FVector Velocity, float StartAge)
	{
		InitOver = false;
		...
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.SpecAtt:
			...
			{
				Velocity.ToDirectionAndLength(out var _, out var length2);
				CurFrameValue = length2;
			}
			break;
		...
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.SpecAttUCurve:
		{
			...
			CurFrameValue = SourceStruct.Curve.GetFloatValue(length);
			break;
		}
		default:
			CurFrameValue = 0f;
	...
	public virtual void UpdateValue(float DeltaTime, FVector Velocity)
	{
		...
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.Random:
			...
			{
				CurFrameValue = MathLib.Lerp(SourceStruct.RangeMin, SourceStruct.RangeMax, FMath.FRand());
			}
			break;
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.TimedUCurve:
		{
			...
			CurFrameValue = SourceStruct.Curve.GetFloatValue(inTime) * SourceStruct.Scale;
			break;
		}
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.SpecAttUCurve:
		{
			...
			{
				Velocity.ToDirectionAndLength(out var _, out var length);
				num = FMath.FInterpTo(num, length, DeltaTime, SourceStruct.Value);
			}
