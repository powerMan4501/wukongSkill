...
public class ModifyNiagaraParamsFVectorInfo
{
	private BUC_DispLibUtil_ModifyNiagaraParams_FVector SourceStruct;

	public bool InitOver { get; protected set; }

	public bool isEndStage { get; protected set; }

	public FVector CurFrameValue { get; protected set; }

	public bool SetEveryFrame { get; protected set; }

	public FVector ValueOnEndStageBegin { get; protected set; }

	public float LoopAge { get; protected set; }

	public float Age { get; protected set; }

	public ModifyNiagaraParamsFVectorInfo(BUC_DispLibUtil_ModifyNiagaraParams_FVector _SourceStruct, FVector Velocity, float StartAge)
	{
	...
	protected virtual void UpdateEndStage()
	{
		...
		{
			CurFrameValue = MathLib.Lerp(ValueOnEndStageBegin, in SourceStruct.EndValue, MathLib.Clamp(LoopAge / SourceStruct.EndStageDuration, 0f, 1f));
		}
	}

	protected virtual void Init(BUC_DispLibUtil_ModifyNiagaraParams_FVector _SourceStruct, FVector Velocity, float StartAge)
	{
		InitOver = false;
		...
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.TimedUCurve:
			...
			CurFrameValue = SourceStruct.Curve.GetVectorValue(0f);
			break;
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.SpecAttUCurve:
		{
			if (SourceStruct.Curve.IsNullOrDestroyed())
			...
			CurFrameValue = SourceStruct.Curve.GetVectorValue(length);
			break;
		}
		default:
			CurFrameValue = FVector.ZeroVector;
			break;
		}
		InitOver = true;
	...
	public virtual void UpdateValue(float DeltaTime, FVector Velocity)
	{
		...
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.Random:
			...
			{
				CurFrameValue = MathLib.Lerp(in SourceStruct.RangeMin, in SourceStruct.RangeMax, FMath.FRand());
			}
			break;
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.TimedUCurve:
		{
			...
			CurFrameValue = SourceStruct.Curve.GetVectorValue(inTime) * SourceStruct.Scale;
			break;
		}
		case DispLibDBCAdvProcessModifyNiagaraParamUtilMode.SpecAttUCurve:
		{
			...
			{
				Velocity.ToDirectionAndLength(out var _, out var length);
				num = FMath.FInterpTo(num, length, DeltaTime, SourceStruct.Value.X);
			}
			CurFrameValue = SourceStruct.Curve.GetVectorValue(num) * SourceStruct.Scale;
			break;
		}
		}
	}
