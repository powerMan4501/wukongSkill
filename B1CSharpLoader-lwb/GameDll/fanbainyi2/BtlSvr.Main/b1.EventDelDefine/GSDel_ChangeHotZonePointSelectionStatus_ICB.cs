using System;
using System.Collections.Generic;
using b1.ECS;

namespace b1.EventDelDefine;

public class GSDel_ChangeHotZonePointSelectionStatus_ICB : IFreshAbleGSEvent
{
	private Del_ChangeHotZonePointSelectionStatus _InvokeCallBack;

	private Action<Del_ChangeHotZonePointSelectionStatus> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_ChangeHotZonePointSelectionStatus _MultiCastDel;

	public GSDel_ChangeHotZonePointSelectionStatus_ICB(Del_ChangeHotZonePointSelectionStatus InvokeCallBack, Action<Del_ChangeHotZonePointSelectionStatus> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_ChangeHotZonePointSelectionStatus)Delegate.Combine(_MultiCastDel, (Del_ChangeHotZonePointSelectionStatus)Del);
		}
		else
		{
			_MultiCastDel = (Del_ChangeHotZonePointSelectionStatus)Delegate.Remove(_MultiCastDel, (Del_ChangeHotZonePointSelectionStatus)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_ChangeHotZonePointSelectionStatus_ICB operator +(GSDel_ChangeHotZonePointSelectionStatus_ICB GSEvent, Del_ChangeHotZonePointSelectionStatus Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_ChangeHotZonePointSelectionStatus)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_ChangeHotZonePointSelectionStatus_ICB operator -(GSDel_ChangeHotZonePointSelectionStatus_ICB GSEvent, Del_ChangeHotZonePointSelectionStatus Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_ChangeHotZonePointSelectionStatus == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_ChangeHotZonePointSelectionStatus)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(EGroupAIHotZoneType HZType, HotZonePointInfo PointInfo, bool IsSelect)
	{
		_InvokeCallBack?.Invoke(HZType, PointInfo, IsSelect);
	}
}
