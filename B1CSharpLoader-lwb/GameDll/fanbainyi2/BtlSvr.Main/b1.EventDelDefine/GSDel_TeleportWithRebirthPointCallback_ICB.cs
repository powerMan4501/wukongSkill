using System;
using System.Collections.Generic;
using b1.ECS;

namespace b1.EventDelDefine;

public class GSDel_TeleportWithRebirthPointCallback_ICB : IFreshAbleGSEvent
{
	private Del_TeleportWithRebirthPointCallback _InvokeCallBack;

	private Action<Del_TeleportWithRebirthPointCallback> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_TeleportWithRebirthPointCallback _MultiCastDel;

	public GSDel_TeleportWithRebirthPointCallback_ICB(Del_TeleportWithRebirthPointCallback InvokeCallBack, Action<Del_TeleportWithRebirthPointCallback> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_TeleportWithRebirthPointCallback)Delegate.Combine(_MultiCastDel, (Del_TeleportWithRebirthPointCallback)Del);
		}
		else
		{
			_MultiCastDel = (Del_TeleportWithRebirthPointCallback)Delegate.Remove(_MultiCastDel, (Del_TeleportWithRebirthPointCallback)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_TeleportWithRebirthPointCallback_ICB operator +(GSDel_TeleportWithRebirthPointCallback_ICB GSEvent, Del_TeleportWithRebirthPointCallback Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_TeleportWithRebirthPointCallback)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_TeleportWithRebirthPointCallback_ICB operator -(GSDel_TeleportWithRebirthPointCallback_ICB GSEvent, Del_TeleportWithRebirthPointCallback Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_TeleportWithRebirthPointCallback == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_TeleportWithRebirthPointCallback)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(int RebirthPointConfigID, ETeleportReason TeleportReason)
	{
		_InvokeCallBack?.Invoke(RebirthPointConfigID, TeleportReason);
	}
}
