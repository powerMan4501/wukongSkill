using System;
using System.Collections.Generic;
using b1.ECS;
using UnrealEngine.Engine;
using UnrealEngine.Runtime;

namespace b1.EventDelDefine;

public class GSDel_SetSkillBaseTarget_ICB : IFreshAbleGSEvent
{
	private Del_SetSkillBaseTarget _InvokeCallBack;

	private Action<Del_SetSkillBaseTarget> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_SetSkillBaseTarget _MultiCastDel;

	public GSDel_SetSkillBaseTarget_ICB(Del_SetSkillBaseTarget InvokeCallBack, Action<Del_SetSkillBaseTarget> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_SetSkillBaseTarget)Delegate.Combine(_MultiCastDel, (Del_SetSkillBaseTarget)Del);
		}
		else
		{
			_MultiCastDel = (Del_SetSkillBaseTarget)Delegate.Remove(_MultiCastDel, (Del_SetSkillBaseTarget)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_SetSkillBaseTarget_ICB operator +(GSDel_SetSkillBaseTarget_ICB GSEvent, Del_SetSkillBaseTarget Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_SetSkillBaseTarget)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_SetSkillBaseTarget_ICB operator -(GSDel_SetSkillBaseTarget_ICB GSEvent, Del_SetSkillBaseTarget Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_SetSkillBaseTarget == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_SetSkillBaseTarget)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(AActor SkillBaseTarget, FVector PointLocation, ETargetSourceType SourceType, string SceneCompName)
	{
		_InvokeCallBack?.Invoke(SkillBaseTarget, PointLocation, SourceType, SceneCompName);
	}
}
