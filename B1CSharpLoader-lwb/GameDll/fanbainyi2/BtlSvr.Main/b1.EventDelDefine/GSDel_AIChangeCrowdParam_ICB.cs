using System;
using System.Collections.Generic;
using b1.ECS;
using BtlB1;

namespace b1.EventDelDefine;

public class GSDel_AIChangeCrowdParam_ICB : IFreshAbleGSEvent
{
	private Del_AIChangeCrowdParam _InvokeCallBack;

	private Action<Del_AIChangeCrowdParam> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_AIChangeCrowdParam _MultiCastDel;

	public GSDel_AIChangeCrowdParam_ICB(Del_AIChangeCrowdParam InvokeCallBack, Action<Del_AIChangeCrowdParam> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_AIChangeCrowdParam)Delegate.Combine(_MultiCastDel, (Del_AIChangeCrowdParam)Del);
		}
		else
		{
			_MultiCastDel = (Del_AIChangeCrowdParam)Delegate.Remove(_MultiCastDel, (Del_AIChangeCrowdParam)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_AIChangeCrowdParam_ICB operator +(GSDel_AIChangeCrowdParam_ICB GSEvent, Del_AIChangeCrowdParam Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_AIChangeCrowdParam)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_AIChangeCrowdParam_ICB operator -(GSDel_AIChangeCrowdParam_ICB GSEvent, Del_AIChangeCrowdParam Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_AIChangeCrowdParam == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_AIChangeCrowdParam)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(float SeparationWeight, float CollisionQueryRange, float PathOptimizationRange, EUnitAICrowdQualityLevel AvoidanceQualityLevel)
	{
		_InvokeCallBack?.Invoke(SeparationWeight, CollisionQueryRange, PathOptimizationRange, AvoidanceQualityLevel);
	}
}
