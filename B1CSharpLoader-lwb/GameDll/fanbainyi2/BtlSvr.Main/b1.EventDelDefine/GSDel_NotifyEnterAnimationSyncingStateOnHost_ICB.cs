using System;
using System.Collections.Generic;
using b1.ECS;
using UnrealEngine.Engine;

namespace b1.EventDelDefine;

public class GSDel_NotifyEnterAnimationSyncingStateOnHost_ICB : IFreshAbleGSEvent
{
	private Del_NotifyEnterAnimationSyncingStateOnHost _InvokeCallBack;

	private Action<Del_NotifyEnterAnimationSyncingStateOnHost> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_NotifyEnterAnimationSyncingStateOnHost _MultiCastDel;

	public GSDel_NotifyEnterAnimationSyncingStateOnHost_ICB(Del_NotifyEnterAnimationSyncingStateOnHost InvokeCallBack, Action<Del_NotifyEnterAnimationSyncingStateOnHost> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_NotifyEnterAnimationSyncingStateOnHost)Delegate.Combine(_MultiCastDel, (Del_NotifyEnterAnimationSyncingStateOnHost)Del);
		}
		else
		{
			_MultiCastDel = (Del_NotifyEnterAnimationSyncingStateOnHost)Delegate.Remove(_MultiCastDel, (Del_NotifyEnterAnimationSyncingStateOnHost)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_NotifyEnterAnimationSyncingStateOnHost_ICB operator +(GSDel_NotifyEnterAnimationSyncingStateOnHost_ICB GSEvent, Del_NotifyEnterAnimationSyncingStateOnHost Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_NotifyEnterAnimationSyncingStateOnHost)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_NotifyEnterAnimationSyncingStateOnHost_ICB operator -(GSDel_NotifyEnterAnimationSyncingStateOnHost_ICB GSEvent, Del_NotifyEnterAnimationSyncingStateOnHost Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_NotifyEnterAnimationSyncingStateOnHost == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_NotifyEnterAnimationSyncingStateOnHost)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(List<int> AnimationSyncStateBuffList, UAnimMontage AnimationSyncMontage)
	{
		_InvokeCallBack?.Invoke(AnimationSyncStateBuffList, AnimationSyncMontage);
	}
}
