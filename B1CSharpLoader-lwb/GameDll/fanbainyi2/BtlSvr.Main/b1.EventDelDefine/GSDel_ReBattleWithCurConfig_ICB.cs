using System;
using System.Collections.Generic;
using b1.ECS;
using ResB1;

namespace b1.EventDelDefine;

public class GSDel_ReBattleWithCurConfig_ICB : IFreshAbleGSEvent
{
	private Del_ReBattleWithCurConfig _InvokeCallBack;

	private Action<Del_ReBattleWithCurConfig> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_ReBattleWithCurConfig _MultiCastDel;

	public GSDel_ReBattleWithCurConfig_ICB(Del_ReBattleWithCurConfig InvokeCallBack, Action<Del_ReBattleWithCurConfig> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_ReBattleWithCurConfig)Delegate.Combine(_MultiCastDel, (Del_ReBattleWithCurConfig)Del);
		}
		else
		{
			_MultiCastDel = (Del_ReBattleWithCurConfig)Delegate.Remove(_MultiCastDel, (Del_ReBattleWithCurConfig)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_ReBattleWithCurConfig_ICB operator +(GSDel_ReBattleWithCurConfig_ICB GSEvent, Del_ReBattleWithCurConfig Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_ReBattleWithCurConfig)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_ReBattleWithCurConfig_ICB operator -(GSDel_ReBattleWithCurConfig_ICB GSEvent, Del_ReBattleWithCurConfig Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_ReBattleWithCurConfig == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_ReBattleWithCurConfig)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(EBossRushType NewBossRushType = EBossRushType.None)
	{
		_InvokeCallBack?.Invoke(NewBossRushType);
	}
}
