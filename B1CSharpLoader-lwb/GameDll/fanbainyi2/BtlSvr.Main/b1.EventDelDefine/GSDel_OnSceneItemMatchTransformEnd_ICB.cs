using System;
using System.Collections.Generic;
using b1.ECS;

namespace b1.EventDelDefine;

public class GSDel_OnSceneItemMatchTransformEnd_ICB : IFreshAbleGSEvent
{
	private Del_OnSceneItemMatchTransformEnd _InvokeCallBack;

	private Action<Del_OnSceneItemMatchTransformEnd> _MultiCastDelChgCallBack;

	public int EventId;

	public GSEventCollectionBase _OwnerEventCollectionCS;

	private Del_OnSceneItemMatchTransformEnd _MultiCastDel;

	public GSDel_OnSceneItemMatchTransformEnd_ICB(Del_OnSceneItemMatchTransformEnd InvokeCallBack, Action<Del_OnSceneItemMatchTransformEnd> MultiCastDelChgCallBack)
	{
		_InvokeCallBack = InvokeCallBack;
		_MultiCastDelChgCallBack = MultiCastDelChgCallBack;
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public void RefreshEntityComponentEventReg(bool Active, Delegate Del)
	{
		if (Active)
		{
			_MultiCastDel = (Del_OnSceneItemMatchTransformEnd)Delegate.Combine(_MultiCastDel, (Del_OnSceneItemMatchTransformEnd)Del);
		}
		else
		{
			_MultiCastDel = (Del_OnSceneItemMatchTransformEnd)Delegate.Remove(_MultiCastDel, (Del_OnSceneItemMatchTransformEnd)Del);
		}
		_MultiCastDelChgCallBack(_MultiCastDel);
	}

	public static GSDel_OnSceneItemMatchTransformEnd_ICB operator +(GSDel_OnSceneItemMatchTransformEnd_ICB GSEvent, Del_OnSceneItemMatchTransformEnd Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			bool flag = entityComponent.IsNetActive();
			orAllocEventRegCache.Add(EventRegCache.AllocEventRegCache(GSEvent.EventId, Del, flag));
			if (!flag)
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_OnSceneItemMatchTransformEnd)Delegate.Combine(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public static GSDel_OnSceneItemMatchTransformEnd_ICB operator -(GSDel_OnSceneItemMatchTransformEnd_ICB GSEvent, Del_OnSceneItemMatchTransformEnd Del)
	{
		if (Del.Target is IEntityComponent entityComponent)
		{
			List<EventRegCache> orAllocEventRegCache = GSEvent._OwnerEventCollectionCS.GetOrAllocEventRegCache(entityComponent);
			for (int num = orAllocEventRegCache.Count - 1; num >= 0; num--)
			{
				if (orAllocEventRegCache[num].Del as Del_OnSceneItemMatchTransformEnd == Del)
				{
					EventRegCache.DeAllocEventRegCache(orAllocEventRegCache[num]);
					orAllocEventRegCache.RemoveAt(num);
				}
			}
			if (orAllocEventRegCache.Count == 0)
			{
				GSEvent._OwnerEventCollectionCS.DeAllocEventRegCache(entityComponent);
			}
			if (!entityComponent.IsNetActive())
			{
				return GSEvent;
			}
		}
		GSEvent._MultiCastDel = (Del_OnSceneItemMatchTransformEnd)Delegate.Remove(GSEvent._MultiCastDel, Del);
		GSEvent._MultiCastDelChgCallBack(GSEvent._MultiCastDel);
		return GSEvent;
	}

	public void Invoke(int MontageID, int NotifyID, bool bForceMatch, bool bBowl)
	{
		_InvokeCallBack?.Invoke(MontageID, NotifyID, bForceMatch, bBowl);
	}
}
