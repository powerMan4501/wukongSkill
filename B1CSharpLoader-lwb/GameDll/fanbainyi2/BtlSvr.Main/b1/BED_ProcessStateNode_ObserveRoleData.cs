...
[DisplayName("观察玩家数据")]
[UClass]
[USharpPath("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData")]
public class BED_ProcessStateNode_ObserveRoleData : BED_ProcessStateNode
{
	...
	[UProperty]
	[USharpPath("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:ObserveMinSuccessConditionNum")]
	public int ObserveMinSuccessConditionNum
	{
		get
		{
			...
			{
				NativeReflection.LogInvalidPropertyAccessed("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:ObserveMinSuccessConditionNum");
				return 0;
			}
	...
	[BlueprintReadWrite]
	[DisplayName("观察信息列表")]
	[USharpPath("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:ObserveRoleDataInfoList")]
	protected TArrayReadWrite<FObserveRoleDataInfo> ObserveRoleDataInfoList
	{
		get
		{
			...
			{
				NativeReflection.LogInvalidPropertyAccessed("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:ObserveRoleDataInfoList");
				return null;
			}
	...
	[USharpPath("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:GetNodeDescriptionInCS")]
	protected override string GetNodeDescriptionInCS_Implementation()
	{
		...
		foreach (FObserveRoleDataInfo observeRoleDataInfo in ObserveRoleDataInfoList)
		{
			...
			case ERoleDataType.ConsumedItem:
				flag = true;
				stringBuilder.Append($"{arg}观察消耗物品，物品ID是：{observeRoleDataInfo.ConsumeItemID}，消耗数量大于：{observeRoleDataInfo.ConsumptionQuantity}");
				break;
			case ERoleDataType.OwningItem:
				flag = true;
				stringBuilder.Append($"{arg}观察拥有物品，物品ID是：{observeRoleDataInfo.OwningItemID}，拥有数量大于：{observeRoleDataInfo.OwningNum}");
				break;
			case ERoleDataType.InteractionRecord:
				flag = true;
				stringBuilder.Append($"{arg}观察交互记录，交互组ID是：{observeRoleDataInfo.InteractionGroupID}，交互步骤大于：{observeRoleDataInfo.InteractionStep}");
				break;
			case ERoleDataType.TaskStage:
				flag = true;
				stringBuilder.Append($"{arg}观察任务阶段，阶段ID是：{observeRoleDataInfo.NeedObserveTaskStageID}, 阶段状态是：{observeRoleDataInfo.ObserveTaskStageState}");
				break;
			}
			if (flag)
			{
	...
	{
		return "Process.State.ObserveRoleData";
	}
	...
	public override bool CheckNodeQueryable(out string ErrorMassager)
	{
		...
		foreach (FObserveRoleDataInfo DataInfo in ObserveRoleDataInfoList)
		{
			...
			case ERoleDataType.OwningItem:
				...
				break;
			case ERoleDataType.TaskStage:
				if (!allTaskStageDesc.ContainsKey(DataInfo.NeedObserveTaskStageID))
				{
					ErrorMassager += $"不存在的任务阶段ID({DataInfo.NeedObserveTaskStageID})";
				}
				break;
			}
		}
	...
	[UFunctionInvoker("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:GetNodeDescriptionInCS")]
	private static void GetNodeDescriptionInCS__Invoker(IntPtr buffer, IntPtr obj)
	{
		BED_ProcessStateNode_ObserveRoleData bED_ProcessStateNode_ObserveRoleData = GCHelper.Find<BED_ProcessStateNode_ObserveRoleData>(obj);
		string nodeDescriptionInCS_Implementation = bED_ProcessStateNode_ObserveRoleData.GetNodeDescriptionInCS_Implementation();
		FStringMarshaler.ToNative(IntPtr.Add(buffer, GetNodeDescriptionInCS_ReturnValue_Offset), nodeDescriptionInCS_Implementation);
	}

	private static void LoadNativeType()
	{
		IntPtr intPtr = NativeReflection.GetClass("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData");
		...
		NativeReflection.LogFunctionIsValid("/Script/b1-Managed.BED_ProcessStateNode_ObserveRoleData:GetNodeDescriptionInCS", GetNodeDescriptionInCS_IsValid);
	}

	static BED_ProcessStateNode_ObserveRoleData()
	{
		if (UnrealTypes.CanLazyLoadManagedType(typeof(BED_ProcessStateNode_ObserveRoleData)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(BED_ProcessStateNode_ObserveRoleData));
	}
}
