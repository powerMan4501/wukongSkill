...
public class BUAFabaoEnergy : BUAttrCostTemplate
{
	public override void DoCostAttrValue(AActor Owner, IBUC_AttrContainer AttrContainer, float AttrCostBase, float AttrCostRatio)
	{
		...
		if (readOnlyData != null)
		{
			...
			if ((!readOnlyData.HasSimpleState(EBGUSimpleState.FabaoEnergyLock) || !(item > 0f)) && (!readOnlyData.HasSimpleState(EBGUSimpleState.CantGainFabaoEnergy) || !(item < 0f)))
			{
				BUS_EventCollectionCS.Get(Owner).Evt_IncreaseAttrFloat.Invoke(EBGUAttrFloat.FabaoEnergy, 0f - item);
			}
		}
	}
	...
	public override bool IsAttrValueEnough(IBUC_AttrContainer AttrContainer, float AttrCostBase, float AttrCostRatio)
	{
		...
		return AttrContainer.GetFloatValue(EBGUAttrFloat.FabaoEnergy) >= GetCostValue(AttrContainer, AttrCostBase, AttrCostRatio).Item2;
	}
	...
	public override (int, float) GetCostValue(IBUC_AttrContainer AttrContainer, float AttrCostBase, float AttrCostRatio)
	{
		...
		return (0, AttrCostBase + AttrCostRatio * AttrContainer.GetFloatValMax(EBGUAttrFloat.FabaoEnergy));
	}
}
