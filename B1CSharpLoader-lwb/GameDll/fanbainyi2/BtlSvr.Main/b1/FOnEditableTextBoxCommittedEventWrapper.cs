...
[UClass]
[USharpPath("/Script/b1-Managed.FOnEditableTextBoxCommittedEventWrapper")]
public class FOnEditableTextBoxCommittedEventWrapper : UILRuntimeDelegateWrapperBase
{
	...
	{
		return new UEditableTextBox.FOnEditableTextBoxCommittedEvent.Signature(callback);
	}

	[UFunction]
	[USharpPath("/Script/b1-Managed.FOnEditableTextBoxCommittedEventWrapper:callback")]
	private void callback(FText Text, ETextCommit CommitMethod)
	{
		((UEditableTextBox.FOnEditableTextBoxCommittedEvent.Signature)_action)(Text, CommitMethod);
	}

	[UFunctionInvoker("/Script/b1-Managed.FOnEditableTextBoxCommittedEventWrapper:callback")]
	private static void callback__Invoker(IntPtr buffer, IntPtr obj)
	{
		FOnEditableTextBoxCommittedEventWrapper fOnEditableTextBoxCommittedEventWrapper = GCHelper.Find<FOnEditableTextBoxCommittedEventWrapper>(obj);
		...
		fOnEditableTextBoxCommittedEventWrapper.callback(text, commitMethod);
	}

	private static void LoadNativeType()
	{
		IntPtr unrealClass = NativeReflection.GetClass("/Script/b1-Managed.FOnEditableTextBoxCommittedEventWrapper");
		...
		NativeReflection.LogFunctionIsValid("/Script/b1-Managed.FOnEditableTextBoxCommittedEventWrapper:callback", callback_IsValid);
	}

	static FOnEditableTextBoxCommittedEventWrapper()
	{
		if (UnrealTypes.CanLazyLoadManagedType(typeof(FOnEditableTextBoxCommittedEventWrapper)))
		{
			LoadNativeType();
		}
		UnrealTypes.OnCCtorCalled(typeof(FOnEditableTextBoxCommittedEventWrapper));
	}
}
