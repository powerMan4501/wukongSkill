...
public class TestState_ATPAllUnitDeadTest : AutoTestNodeLib.TestState_Node
{
	...
	public TestState_ATPAllUnitDeadTest(UObject WorldContext, AutoTestHelperLib.BGWDataAsset_TamerListConfig TamerListConfig, bool QuitBiuPerUnit, int DistanceBetween, int BUFF)
	{
	...
	public override AutoTestNodeLib.NodeState OnTick(out string Description, ref Stack<AutoTestNodeLib.TestState_Node> StateStack, float DeltaTime)
	{
		...
		case InnerState.AddTest:
		{
			...
			ACameraActor frontCam = null;
			ACameraActor[] allActorsOfClass = UGameplayStatics.GetAllActorsOfClass<ACameraActor>(WorldContext);
			foreach (ACameraActor aCameraActor in allActorsOfClass)
			{
				if (aCameraActor.GetName().Contains("CameraActor_1"))
				{
					frontCam = aCameraActor;
					break;
				}
			}
			List<AutoTestNodeLib.TestState_Node> list3 = new List<AutoTestNodeLib.TestState_Node>
			{
				new AutoTestNodeLib.TestState_Wait(WorldContext, 1),
				...
				new AutoTestNodeLib.TestState_Wait(WorldContext, 1),
				new TestState_RecordUnitDeadtart(WorldContext),
				new TestState_UnitDeadPerformance(WorldContext, TamerListConfig.TamerList[StartIndex], DistanceBetween, frontCam, BUFF),
				new TestState_RecordUnitDead2File(WorldContext, RecordFilePath, text),
